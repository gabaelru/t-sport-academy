        ...
        <section className="py-12">
          ...
          {posts.length === 0 ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              {posts.map((post) => (
                <article key={post.slug} className="flex flex-col md:flex-row items-start bg-card p-4 rounded-lg shadow-sm gap-4">
                  {post.image ? (
                    <div className="w-full md:w-40 h-40 md:h-28 overflow-hidden rounded-md flex-shrink-0 flex items-center justify-center bg-muted/40">
                      <img src={post.image} alt={post.title} className="max-w-full max-h-full object-contain" />
                    </div>
                  ) : null}
